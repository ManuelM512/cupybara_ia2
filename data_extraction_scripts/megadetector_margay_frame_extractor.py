#!/usr/bin/env python3
"""
megadetector_margay_frame_extractor.py
======================================
Extract frames containing animals from videos labeled as 'margay' in test.csv
--------------------------------------------------------------------

This script processes only videos from `dataset/test` that are labeled as "margay" 
...
Frames are saved to:
    <output_dir>/
        train/margay/*.jpg   # 80% of margay videos
        val/margay/*.jpg     # 10% of margay videos  
        test/margay/*.jpg    # 10% of margay videos
...
------
```bash
python megadetector_margay_frame_extractor.py \
        --csv_path test.csv \
...
def process_margay_videos(
    ...
    *,
    ...
    """Process margay videos and extract frames with animal detections."""
    ...
    for split, video_list in video_splits.items():
        ...
        split_output_dir = output_dir / split / "margay"
    ...
    print(f"\n🎉 Processing complete! Total margay frames saved: {total_frames_saved}")
...
def main():
    parser = argparse.ArgumentParser(description="Extract animal frames from margay-labeled videos using MegaDetector")
    ...
    parser.add_argument("--frames_per_video", type=int, default=140,
    ...
    # Read margay videos from CSV
    print("📋 Reading CSV for margay-labeled videos...")
    margay_videos = read_csv_for_species(csv_path, "margay")
    print(f"📹 Found {len(margay_videos)} videos labeled as 'margay'")
    
    if not margay_videos:
        print("❌ No margay videos found in CSV. Exiting.")
        return
    
    # Split videos into train/val/test
    rng = random.Random(args.seed)
    video_splits = split_videos_80_10_10(margay_videos, rng)
