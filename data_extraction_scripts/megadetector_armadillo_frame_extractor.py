#!/usr/bin/env python3
"""
megadetector_armadillo_frame_extractor.py
=========================================
Extract frames containing animals from videos labeled as 'armadillo' in test.csv
--------------------------------------------------------------------

This script processes only videos from `dataset/test` that are labeled as "armadillo" 
...
Frames are saved to:
    <output_dir>/
        train/armadillo/*.jpg   # 80% of armadillo videos
        val/armadillo/*.jpg     # 10% of armadillo videos  
        test/armadillo/*.jpg    # 10% of armadillo videos
...
------
```bash
python megadetector_armadillo_frame_extractor.py \
        --csv_path test.csv \
...
def process_armadillo_videos(
    ...
    *,
    ...
    """Process armadillo videos and extract frames with animal detections."""
    ...
    for split, video_list in video_splits.items():
        ...
        split_output_dir = output_dir / split / "armadillo"
    ...
    print(f"\n🎉 Processing complete! Total armadillo frames saved: {total_frames_saved}")
...
def main():
    parser = argparse.ArgumentParser(description="Extract animal frames from armadillo-labeled videos using MegaDetector")
    ...
    # Read armadillo videos from CSV
    print("📋 Reading CSV for armadillo-labeled videos...")
    armadillo_videos = read_csv_for_species(csv_path, "armadillo")
    print(f"📹 Found {len(armadillo_videos)} videos labeled as 'armadillo'")
    
    if not armadillo_videos:
        print("❌ No armadillo videos found in CSV. Exiting.")
        return
    
    # Split videos into train/val/test
    rng = random.Random(args.seed)
    video_splits = split_videos_80_10_10(armadillo_videos, rng)
