#!/usr/bin/env python3
"""
megadetector_hare_frame_extractor.py
===================================
Extract frames containing animals from videos labeled as 'hare' in test.csv
------------------------------------------------------------------------

This script processes only videos from `dataset/test` that are labeled as "hare" 
...
Frames are saved to:
    <output_dir>/
        train/hare/*.jpg   # 80% of hare videos
        val/hare/*.jpg     # 10% of hare videos  
        test/hare/*.jpg    # 10% of hare videos
...
------
```bash
python megadetector_hare_frame_extractor.py \
        --csv_path test.csv \
...
def process_hare_videos(
    ...
    *,
    ...
    """Process hare videos and extract frames with animal detections."""
    ...
    for split, video_list in video_splits.items():
        ...
        split_output_dir = output_dir / split / "hare"
    ...
    print(f"\n🎉 Processing complete! Total hare frames saved: {total_frames_saved}")
...
def main():
    parser = argparse.ArgumentParser(description="Extract animal frames from hare-labeled videos using MegaDetector")
    ...
    parser.add_argument("--frames_per_video", type=int, default=150,
    ...
    # Read hare videos from CSV
    print("📋 Reading CSV for hare-labeled videos...")
    hare_videos = read_csv_for_species(csv_path, "hare")
    print(f"📹 Found {len(hare_videos)} videos labeled as 'hare'")
    
    if not hare_videos:
        print("❌ No hare videos found in CSV. Exiting.")
        return
    
    # Split videos into train/val/test
    rng = random.Random(args.seed)
    video_splits = split_videos_80_10_10(hare_videos, rng)
