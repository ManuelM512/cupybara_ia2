#!/usr/bin/env python3
"""
megadetector_cow_frame_extractor.py
===================================
Extract frames containing animals from videos labeled as 'cow' in test.csv
--------------------------------------------------------------------

This script processes only videos from `dataset/test` that are labeled as "cow" 
...
Frames are saved to:
    <output_dir>/
        train/cow/*.jpg   # 80% of cow videos
        val/cow/*.jpg     # 10% of cow videos  
        test/cow/*.jpg    # 10% of cow videos
...
------
```bash
python megadetector_cow_frame_extractor.py \
        --csv_path test.csv \
...
def process_cow_videos(
    ...
    *,
    ...
    """Process cow videos and extract frames with animal detections."""
    ...
    for split, video_list in video_splits.items():
        ...
        split_output_dir = output_dir / split / "cow"
    ...
    print(f"\n🎉 Processing complete! Total cow frames saved: {total_frames_saved}")
...
def main():
    parser = argparse.ArgumentParser(description="Extract animal frames from cow-labeled videos using MegaDetector")
    ...
    parser.add_argument("--frames_per_video", type=int, default=60,
    ...
    # Read cow videos from CSV
    print("📋 Reading CSV for cow-labeled videos...")
    cow_videos = read_csv_for_species(csv_path, "cow")
    print(f"📹 Found {len(cow_videos)} videos labeled as 'cow'")
    
    if not cow_videos:
        print("❌ No cow videos found in CSV. Exiting.")
        return
    
    # Split videos into train/val/test
    rng = random.Random(args.seed)
    video_splits = split_videos_80_10_10(cow_videos, rng)
