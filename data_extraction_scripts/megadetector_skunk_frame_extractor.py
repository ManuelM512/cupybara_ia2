#!/usr/bin/env python3
"""
megadetector_skunk_frame_extractor.py
=====================================
Extract frames containing animals from videos labeled as 'skunk' in test.csv
--------------------------------------------------------------------

This script processes only videos from `dataset/test` that are labeled as "skunk" 
...
Frames are saved to:
    <output_dir>/
        train/skunk/*.jpg   # 80% of skunk videos
        val/skunk/*.jpg     # 10% of skunk videos  
        test/skunk/*.jpg    # 10% of skunk videos
...
------
```bash
python megadetector_skunk_frame_extractor.py \
        --csv_path test.csv \
...
def process_skunk_videos(
    ...
    *,
    ...
    """Process skunk videos and extract frames with animal detections."""
    ...
    for split, video_list in video_splits.items():
        ...
        split_output_dir = output_dir / split / "skunk"
    ...
    print(f"\n🎉 Processing complete! Total skunk frames saved: {total_frames_saved}")
...
def main():
    parser = argparse.ArgumentParser(description="Extract animal frames from skunk-labeled videos using MegaDetector")
    ...
    parser.add_argument("--frames_per_video", type=int, default=300,
    ...
    # Read skunk videos from CSV
    print("📋 Reading CSV for skunk-labeled videos...")
    skunk_videos = read_csv_for_species(csv_path, "skunk")
    print(f"📹 Found {len(skunk_videos)} videos labeled as 'skunk'")
    
    if not skunk_videos:
        print("❌ No skunk videos found in CSV. Exiting.")
        return
    
    # Split videos into train/val/test
    rng = random.Random(args.seed)
    video_splits = split_videos_80_10_10(skunk_videos, rng)
