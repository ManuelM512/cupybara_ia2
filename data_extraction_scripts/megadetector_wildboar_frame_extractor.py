#!/usr/bin/env python3
"""
megadetector_wildboar_frame_extractor.py
========================================
Extract frames containing animals from videos labeled as 'wild_boar' in test.csv
--------------------------------------------------------------------------

This script processes only videos from `dataset/test` that are labeled as "wild_boar" 
...
Frames are saved to:
    <output_dir>/
        train/wild_boar/*.jpg   # 80% of wild_boar videos
        val/wild_boar/*.jpg     # 10% of wild_boar videos  
        test/wild_boar/*.jpg    # 10% of wild_boar videos
...
------
```bash
python megadetector_wildboar_frame_extractor.py \
        --csv_path test.csv \
...
def process_wildboar_videos(
    ...
    *,
    ...
    """Process wild_boar videos and extract frames with animal detections."""
    ...
    for split, video_list in video_splits.items():
        ...
        split_output_dir = output_dir / split / "wild_boar"
    ...
    print(f"\n🎉 Processing complete! Total wild_boar frames saved: {total_frames_saved}")
...
def main():
    parser = argparse.ArgumentParser(description="Extract animal frames from wild_boar-labeled videos using MegaDetector")
    ...
    parser.add_argument("--frames_per_video", type=int, default=130,
    ...
    # Read wild_boar videos from CSV
    print("📋 Reading CSV for wild_boar-labeled videos...")
    wildboar_videos = read_csv_for_species(csv_path, "wild_boar")
    print(f"📹 Found {len(wildboar_videos)} videos labeled as 'wild_boar'")
    
    if not wildboar_videos:
        print("❌ No wild_boar videos found in CSV. Exiting.")
        return
    
    # Split videos into train/val/test
    rng = random.Random(args.seed)
    video_splits = split_videos_80_10_10(wildboar_videos, rng)
